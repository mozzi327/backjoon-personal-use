package level16_동적계획법1_Clear.Q13_11054_가장긴바이토닉부분수열;

/*
문제) 가장 긴 바이토닉 부분 수열
    수열 S가 어떤 수 Sk를 기준으로 S1 < S2 < ... Sk-1 < Sk > Sk+1 > ... SN-1 > SN을 만족한다면, 그 수열을 바이토닉 수열이라고 한다.
    예를 들어, {10, 20, 30, 25, 20}과 {10, 20, 30, 40}, {50, 40, 25, 10} 은 바이토닉 수열이지만,
    {1, 2, 3, 2, 1, 2, 3, 2, 1}과 {10, 20, 30, 40, 20, 30} 은 바이토닉 수열이 아니다.
    수열 A가 주어졌을 때, 그 수열의 부분 수열 중 바이토닉 수열이면서 가장 긴 수열의 길이를 구하는 프로그램을 작성하시오.

입력)
    첫째 줄에 수열 A의 크기 N이 주어지고, 둘째 줄에는 수열 A를 이루고 있는 Ai가 주어진다.
    (1 ≤ N ≤ 1,000, 1 ≤ Ai ≤ 1,000)

출력)
    첫째 줄에 수열 A의 부분 수열 중에서 가장 긴 바이토닉 수열의 길이를 출력한다.
    시간 제한 : 1초
    메모리 제한 : 256MB

힌트
    예제의 경우 {1 5 2 1 4 3 4 5 2 1}이 가장 긴 바이토닉 부분 수열이다.

*/

import java.io.IOException;

public class Main {
    private static Integer[] l_dp;
    private static Integer[] r_dp;
    private static int[] inputs;

    public static void main(String[] args) throws IOException {
        int N = readInt();
        inputs = new int[N];
        l_dp = new Integer[N];
        r_dp = new Integer[N];

        for (int i = 0; i < N; i++) {
            inputs[i] = readInt();
        }

        for (int i = 0; i < N; i++) {
            lisLeft(i);
            lisRight(i);
        }

        int max = 0;
        for (int i = 0; i < N; i++) {
            max = Math.max(max, l_dp[i] + r_dp[i]);
        }

        System.out.println(max - 1);
    }

    private static int lisLeft(int N) {
        if (l_dp[N] == null) {
            l_dp[N] = 1;
            for (int i = N + 1; i < l_dp.length; i++) {
                if (inputs[i] < inputs[N])
                    l_dp[N] = Math.max(l_dp[N], lisLeft(i) + 1);
            }
        }

        return l_dp[N];
    }

    private static int lisRight(int N) {
        if (r_dp[N] == null) {
            r_dp[N] = 1;
            for (int i = N - 1; i > -1; i--) {
                if (inputs[i] < inputs[N])
                    r_dp[N] = Math.max(r_dp[N], lisRight(i) + 1);
            }
        }

        return r_dp[N];
    }


    private static int readInt() throws IOException {
        boolean isNegative = false;
        int value = 0;
        while (true) {
            int input = System.in.read();
            if (input == '\n' || input == ' ')
                return (isNegative) ? -1 * value : value;
            else if (input == '-') isNegative = true;
            else value = value * 10 + (input - 48);
        }
    }
}
