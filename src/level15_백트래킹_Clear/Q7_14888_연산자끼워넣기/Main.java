package level15_백트래킹_Clear.Q7_14888_연산자끼워넣기;
...
public class Main {
    ...
    public static void recur(int depth, int operNum) {
        if (depth == N) {
            minValue = Math.min(minValue, operNum);
            maxValue = Math.max(maxValue, operNum);
            return;
        }
        ...
        for (int i = 0; i < 4; i++) {
            if (operator[i] > 0) {
                ...
                switch (i) {
                    case 0 : recur(depth + 1, (operNum + num[depth]) % MOD); break;
                    case 1 : recur(depth + 1, (operNum - num[depth]) % MOD); break;
                    case 2 : recur(depth + 1, (operNum * num[depth]) % MOD); break;
                    case 3 : recur(depth + 1, (operNum / num[depth]) % MOD); break;
                }
