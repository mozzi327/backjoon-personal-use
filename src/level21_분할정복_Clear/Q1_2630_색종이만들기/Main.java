package level21_분할정복_Clear.Q1_2630_색종이만들기;
...
public class Main {
    ...
    private static int whiteColorCount = 0;

    public static void main(String[] args) throws IOException {
        int N = readInt();
        colorPaperArr = new int[N][N];
        for (int i = 0; i < N; i++) {
            for (int j = 0; j < N; j++) {
                colorPaperArr[i][j] = readInt();
            }
        }
    ...
    private static void divideAndConquer(int row, int col, int N) {
        if (checkColor(row, col, N)) {
            if (colorPaperArr[row][col] == 1) {
                blueColorCount++;
            } else whiteColorCount++;
            return;
        }

        divideAndConquer(row, col, N/2);
        divideAndConquer(row, col + N/2, N/2);
        divideAndConquer(row+N/2, col, N/2);
        divideAndConquer(row+N/2, col+N/2, N/2);
    }

    private static boolean checkColor(int row, int col, int size) {
        int color = colorPaperArr[row][col];

        for (int i = row; i < row + size; i++) {
            for (int j = col; j < col + size; j++) {
                if (colorPaperArr[i][j] != color) {
                    return false;
                }
            }
        }
