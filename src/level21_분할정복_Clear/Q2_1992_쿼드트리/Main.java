package level21_분할정복_Clear.Q2_1992_쿼드트리;
...
public class Main {
    private static char[][] quadTreeArr;
    private static StringBuilder result = new StringBuilder();

    public static void main(String[] args) throws IOException {
        BufferedReader br = new BufferedReader(new InputStreamReader(System.in));
        int N = Integer.parseInt(br.readLine());
        quadTreeArr = new char[N][N];
        for (int i = 0; i < N; i++) {
            quadTreeArr[i] = br.readLine().toCharArray();
        }
        divideAndConquer(0, 0, N);
        System.out.println(result.toString());
    }

    private static boolean checked(int row, int col, int N) {
        char firstValue = quadTreeArr[row][col];
        for (int i = row; i < row + N; i++) {
            for (int j = col; j < col + N; j++) {
                if(firstValue != quadTreeArr[i][j]) return false;
            }
        }
        return true;
    }
    ...
    private static void divideAndConquer(int row, int col, int N) {
        if (checked(row, col, N)) {
            result.append(quadTreeArr[row][col]);
            return;
        }
        result.append('(');


        divideAndConquer(row, col, N/2); // 왼 , 위
        divideAndConquer(row, col + N/2, N/2); // 오 , 위
        divideAndConquer(row+N/2, col, N/2); // 왼 , 아래
        divideAndConquer(row+N/2, col+N/2, N/2); // 오 , 아래
        result.append(')');
    }
}
